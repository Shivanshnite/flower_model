...
def train_model(train_generator, validation_generator, nb_train_samples, nb_validation_samples):
    ...
    checkpoint = ModelCheckpoint("flowers.h5",
                                 monitor="val_loss",
                                 mode="min",
                                 save_best_only = True,
                                 verbose=1)
...
def load_my_model():
    ...
    resnet = ResNet(weights = 'imagenet', include_top = False, input_shape = (img_rows, img_cols, 3))
...
def load_data():
    train_data_dir = 'flower/17_flowers/train/'
    validation_data_dir = 'flower/17_flowers/test/'
